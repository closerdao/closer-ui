...
const VolunteerOpportunitiesPage = ({ pageMeta }: Props) => {
  ...
  const meta = resolvePageMeta(pageMeta, {
    ...
    description: `Join ${PLATFORM_NAME} as a volunteer. Contribute to regenerative community building and gain hands-on experience.`,
  });
...
VolunteerOpportunitiesPage.getInitialProps = async (
  ...
  const pageMeta = await fetchPageMetaOverride('/volunteer');
  return { pageMeta };
};
