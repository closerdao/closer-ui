...
const StayPage = ({
  ...
  const meta = resolvePageMeta(pageMeta, {
    title: `${t('stay_title')} ${PLATFORM_NAME}`,
    description:
      t('stay_meta_description') ||
      `Book your stay at ${PLATFORM_NAME}. Discover unique accommodations in regenerative communities.`,
  });
...
StayPage.getInitialProps = async (context: NextPageContext) => {
  try {
    ...
    const pageMeta = await fetchPageMetaOverride('/stay');
