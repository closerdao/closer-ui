...
const Events = ({ generalConfig, eventsConfig }: Props) => {
  ...
  const pastEventsByYear = useMemo(() => {
    const groups: {
      [key: string]: { year: string; yearKey: string; events: Event[] };
    } = {};
  ...
  const loadEvents = async () => {
    try {
      ...
      const upcomingRes = await platform.event.get(upcomingFilter);
      const upcomingData = upcomingRes?.results;
      setUpcomingEvents(
        upcomingData
          ? upcomingData.map((e: { toJSON: () => Event }) => e.toJSON())
          : [],
      );
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:title"
          content={`${t('events_title')} - ${PLATFORM_NAME}`}
        />
        <meta
        ...
        <meta
          ...
          content={`Discover upcoming events, workshops, and gatherings at ${PLATFORM_NAME}.`}
        />
        <link
          rel="canonical"
          href={`${
