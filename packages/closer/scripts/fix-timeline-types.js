...
try {
  ...
  if (content.includes('declare const ReactCalendarTimeline')) {
  ...
  const newFunctionDefinition = `declare const ReactCalendarTimeline: <
        CustomItem extends TimelineItemBase<any> = TimelineItemBase<number>,
        CustomGroup extends TimelineGroupBase = TimelineGroupBase,
  ...
  if (content.includes(oldClassDefinition)) {
    ...
    console.log('⚠️  Could not find the expected class definition to replace');
  }
} catch (error) {
