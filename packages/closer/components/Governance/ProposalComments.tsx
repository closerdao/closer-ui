...
const ProposalComments: React.FC<ProposalCommentsProps> = ({ proposal, className = '' }) => {
  ...
  // Load comments
  useEffect(() => {
    if (proposal._id && !hasLoaded.current) {
  ...
  const commentsWithReplies = Array.from(commentsMap.values()).map((comment: any) => {
    const repliesMap = platform.post.find({
      where: {
        parentType: 'post',
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await platform.post.post(commentData);
      setNewComment('');
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-start space-x-3">
            <div className="w-8 h-8 bg-accent rounded-full flex items-center justify-center text-white text-sm font-medium">
              {(author?.screenname || author?.email || 'U').charAt(0).toUpperCase()}
            </div>
            <div className="flex-1">
              ...
              <div className="markdown text-gray-700" style={{ whiteSpace: 'pre' }}>
    ...
    <div className={`bg-white rounded-lg border border-gray-200 p-6 ${className}`}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {commentsWithReplies.map((comment: Comment) => renderComment(comment))}
        </div>
      )}
    </div>
  );
