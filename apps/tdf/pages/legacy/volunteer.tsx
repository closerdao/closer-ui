...
const VolunteerOpportunitiesPage = ({
  ...
  const utilityRate =
    typeof bookingConfig?.utilityFiatVal === 'number'
      ? bookingConfig.utilityFiatVal
      : null;
  const foodRate =
    volunteerFoodPrice ??
    (typeof bookingConfig?.foodPriceBasic === 'number'
      ? bookingConfig.foodPriceBasic
      : null);
  const hasDailyRates = foodRate != null && utilityRate != null;
  const dailyValues = {
    food: foodRate != null ? formatRate(foodRate) : '—',
    utilities: utilityRate != null ? formatRate(utilityRate) : '—',
    total: hasDailyRates ? formatRate(foodRate + utilityRate) : '—',
  };

  const knowValuesFor = (id: string) => {
    if (id === 'minstay' || id === 'resident') {
      return { var: volunteeringMinStay };
    }
    if (id === 'daily') {
      return dailyValues;
    }
