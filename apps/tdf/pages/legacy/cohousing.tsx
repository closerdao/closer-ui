...
const CohousingPage = ({
  ...
  const { BLOCKCHAIN_DAO_TOKEN } = config;
  const isCohousingEnabled = Boolean(
    (config as { cohousing?: { enabled?: boolean } }).cohousing?.enabled,
  );

  const meta = resolvePageMeta(pageMeta, {
    title: t('cohousing_hero_title'),
    description: t('cohousing_page_description'),
...
CohousingPage.getInitialProps = async (_context: NextPageContext) => {
  const pageMeta = await fetchPageMetaOverride('/cohousing');
  return { pageMeta };
};
