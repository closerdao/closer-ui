...
const TeamPage = ({ pageMeta }: { pageMeta?: PageMetaOverride | null }) => {
  ...
  const meta = resolvePageMeta(pageMeta, {
    title: t('team_page_title'),
    description: t('team_page_description'),
  });
...
TeamPage.getInitialProps = async (_context: NextPageContext) => {
  const pageMeta = await fetchPageMetaOverride('/team');
  return { pageMeta };
};
